...
​        => q=1인 경우 모델에 그 변수를 추가하는 것에 대한 부분적 효과 나타냄(단순선형회귀의 경우 t 검정과 같음)
...
​			해당 선형회귀모델에서, 특정 매체에 편향되어 지출하는 경우 sales를 과대추정

​                         =>광고매체 간에 상호작용 효과(synergy)가 있을 것!
...
위의 표는 gender에 대한 balance의 회귀와 관련된 최소제곱 계수 추정치인데, 가변수에 대한 p-값이 매우 높음을 확인할 수 있다. 즉, balance에 있어서 gender 따른 차이가 존재한다는 통계적 증거가 없다는 의미다.
